# Modified from: https://github.com/LeapLabTHU/DAT/blob/main/models/dat_blocks.py
...
class DAttentionBaseline(nn.Module):
    ...
    def _get_ref_points(self, H_key, W_key, B, dtype, device):
        ...
        y_fp32 = torch.linspace(0.5, H_key - 0.5, H_key, dtype=torch.float32, device=device)
        x_fp32 = torch.linspace(0.5, W_key - 0.5, W_key, dtype=torch.float32, device=device)
        ...
        if torch.onnx.is_in_onnx_export():
            # 1) stack your (y, x) coords into [..., 2]
            ref = torch.stack((ref_y, ref_x), dim=-1)
            # 2) split into two tensors
            y, x = torch.unbind(ref, dim=-1)
            # 3) normalize each channel
            x = x / (W_key - 1.0) * 2.0 - 1.0
            y = y / (H_key - 1.0) * 2.0 - 1.0
            # 4) re-stack in the same order (y first, x second)
            ref = torch.stack((y, x), dim=-1)
            # 5) add your batch dimension and expand
            ref = ref.unsqueeze(0).expand(B * self.n_groups, -1, -1, -1)
        else:
    ...
    def _get_q_grid(self, H, W, B, dtype, device):
        ...
        # Build arange in float32, then cast to float16 only if needed
        coords_h_fp32 = torch.arange(0, H, dtype=torch.float32, device=device)
        coords_w_fp32 = torch.arange(0, W, dtype=torch.float32, device=device)

        if dtype == torch.float16:
            y = coords_h_fp32.half()
            x = coords_w_fp32.half()
        else:
            y = coords_h_fp32
            x = coords_w_fp32
    ...
    def forward(self, x):
        ...
        if self.no_off:
            ...
            if torch.onnx.is_in_onnx_export():
                grid = pos.flip(-1)   # flips last dim: [y,x] → [x,y]
        ...
        if self.use_pe and (not self.no_off):
            ...
            if self.dwc_pe:
                ...
                attn_bias = rpe_table.unsqueeze(0).expand(B, -1, -1, -1)
